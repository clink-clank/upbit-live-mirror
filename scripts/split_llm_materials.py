...
if not SRC.exists():
    print("LLM_MATERIALS.json not found; skip split.", file=sys.stderr)
    sys.exit(0)
...
n = len(markets)
if n == 0:
    print("No markets to split.", file=sys.stderr)
    sys.exit(0)
...
for i in range(k):
    ...
    meta["shard"] = {"index": i, "total": k, "range": [a, b - 1], "count": len(shard)}
    out["meta"] = meta
